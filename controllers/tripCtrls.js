...
const createTrip = (req, res) => {
  db.Trip.create(req.body).then((createdTrip) => {
    if (!createdTrip) {
...
const updateTrip = (req, res) => {
  db.Trip.findByIdAndUpdate(req.params.id, req.body, { new: true }).then(
    (updatedTrip) => {
      if (!updatedTrip) {
...
const deleteTrip = (req, res) => {
  db.Trip.findByIdAndDelete(req.params.id).then((deletedTrip) => {
    if (!deletedTrip) {
