...
const createActivity = (req, res) => {
  ...
  .then((createdActivity) => {
    if(!createdActivity) {
      ...
      res.status(201).json({data: createdActivity, message: 'activity created'})
    }
  })
}

const updateActivity = (req, res) => {
  db.Activity.findByIdAndUpdate(req.params.id, req.body, {new: true})
  .then((updatedActivity) => {
    if(!updatedActivity) {
      res.status(400).json({message: 'could not update activity'})
    } else {
      res.status(200).json({data: updatedActivity, message: 'updated activity'})
    }
  })
}
...
const deleteActivity = (req, res) => {
  ...
  .then((deletedActivity) => {
    if(!deletedActivity) {
      ...
      res.status(200).json({data: deletedActivity, message: 'activity deleted'})
    }
  })
}
